...
void main() {
  ...
  group('Phase 2 & 3: RevenueCat Environment Separation & Safety Invariants',
    ...
    test('Debug build selects Test Store key (test_*) when provided', () {
      ...
      expect(PremiumEntitlementService.getKeyDiagnostic(resolved),
          equals('Test Store'));
    });
  ...
  group('Phase 4 & 5: Identity & Entitlement Invariants', () {
    ...
    test('Sign out removes cached entitlement and UID from SharedPreferences',
        () async {
      SharedPreferences.setMockInitialValues({
      ...
      expect(prefsBefore.getString('pet_cached_premium_uid'),
          equals('test_firebase_uid_123'));
  ...
  group('Phase 3: Diagnostic Safety — Key Privacy', () {
    test(
        ...
        () {
      expect(PremiumEntitlementService.getKeyDiagnostic(''),
          equals('Unconfigured'));
      expect(PremiumEntitlementService.getKeyDiagnostic('REPLACE_ME'),
          equals('Unconfigured'));
      expect(PremiumEntitlementService.getKeyDiagnostic('test_abc123xyz'),
          equals('Test Store'));
      expect(PremiumEntitlementService.getKeyDiagnostic('goog_abc123xyz'),
          equals('Android production'));
      expect(PremiumEntitlementService.getKeyDiagnostic('appl_abc123xyz'),
          equals('iOS production'));
      expect(PremiumEntitlementService.getKeyDiagnostic('sk_secret123'),
          equals('Secret Key (UNSAFE)'));
      expect(PremiumEntitlementService.getKeyDiagnostic('custom_unknown_key'),
          equals('Custom / Other'));

      // Ensure diagnostic string never contains secret fragments
      final diagnostic = PremiumEntitlementService.getKeyDiagnostic(
          'test_super_secret_payload');
