  ...
  group('Production Launch Remediation & Security Invariant Tests', () {
    test('PET-01: Target SDK is 36 in build.gradle.kts', () {
      ...
      expect(buildGradle.contains('targetSdk = 36'), isTrue,
          reason:
              'Google Play requires targetSdk = 36 for August 2026 submissions.');
      expect(buildGradle.contains('compileSdk = 36'), isTrue);
    });

    test(
        'PET-02: Firestore security rules cover all core and premium collections with strict ownership',
        () {
      ...
      expect(
          rulesFile.contains('request.auth != null && request.auth.uid == uid'),
          isTrue);
      ...
      for (final collection in requiredCollections) {
        expect(
          ...
          isTrue,
          reason:
              'firestore.rules must contain explicit match block for $collection',
        );
      }
    ...
    test(
        'PET-03: Cloudflare Worker enforces server-side system prompt and sanitizes context injection',
        () {
      final workerSource =
          File('cloudflare_worker/src/index.js').readAsStringSync();
    ...
    test(
        'PET-05: External Account Deletion page exists and meets Play Store requirements',
        () {
    ...
    test(
        'Financial Data Invariant: PII redaction cleans sensitive numbers from note/merchant strings',
        () {
      const rawInfo =
          'Card ending 5123456789012345, Phone: 9876543210, Account 123456789012';
      ...
      expect(redactedAccount.contains('XX1234'),
          isTrue); // Pre-masked form preserved
    });

    test(
        'Model Serialization: RecurringRule and Transaction toFirestore formats adhere to schema',
        () {
