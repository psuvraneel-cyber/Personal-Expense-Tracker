...
void main() {
  ...
  group('STRESS TEST STAGE 1: Massive Concurrent Scan & Insertion', () {
    test(
      '15,000 Bank SMS + 5,000 Duplicates Concurrent Ingestion: Zero Duplicates & Consistent State',
      () async {
        ...
        for (int i = 0; i < 15000; i++) {
          ...
          final body =
              'Rs. ${(i + 1) * 10}.00 debited from A/C XX${1000 + (i % 8999)} at MERCHANT_$i via UPI Ref 61234$i.';
        ...
        expect(
          storedTxns.length,
          equals(15000),
          reason: 'Must store exactly 15,000 unique transactions',
        );
        expect(
          stateRows.length,
          equals(15000),
          reason: 'Processing state must contain exactly 15,000 unique rows',
        );
        ...
        print(
          '[STRESS TEST STAGE 1 PASSED] 20,000 concurrent item insertions completed in ${stopwatch.elapsedMilliseconds}ms. Unique items: ${storedTxns.length}',
        );
      },
    );
  });
  ...
  group('STRESS TEST STAGE 2 & 3: Delete & Ignore While Parsing Race', () {
    test('Concurrent Delete/Ignore vs Continuous Ingestion (500 Iterations)', () async {
      ...
      for (int iteration = 0; iteration < 500; iteration++) {
        ...
        final body =
            'Rs. ${100 + iteration}.00 debited from A/C XX9999 at STORE_$iteration via UPI Ref 99$iteration';
        final hash = SmsTransaction.generateHash(body, timestamp);
        ...
        final currentTxns = await db.query(
          'sms_transactions',
          where: 'id = ?',
          whereArgs: [txnId],
        );
        final stateRow = await db.query(
        ...
        expect(
          status == 'deleted' || status == 'ignored' || status == 'accepted',
          isTrue,
        );
        ...
        if (userActionIsDelete) {
          expect(
            ...
            isEmpty,
            reason:
                'Deleted transaction must not exist in sms_transactions table',
          );
        }
      }

      print(
        '[STRESS TEST STAGE 2 & 3 PASSED] 500 concurrent delete/ignore vs re-scan iterations succeeded with zero race condition violations.',
      );
    });
  });
  ...
  group('STRESS TEST STAGE 5: Notification Listener vs Inbox Scan Race', () {
    test(
      'Simultaneous Notification & Inbox Ingestion of Same Transaction (1,000 Iterations)',
      () async {
        for (int i = 0; i < 1000; i++) {
          ...
          expect(
            successCount,
            equals(1),
            reason: 'Exactly one source must succeed in race condition',
          );
        ...
        print(
          '[STRESS TEST STAGE 5 PASSED] 1,000 Notification vs Inbox race tests passed with 100% single-winner invariant enforcement.',
        );
      },
    );
  });
  ...
  group('STRESS TEST STAGE 6: Rapid Refresh Spam', () {
    test(
      'Simulate 20 Refresh Calls per Second Concurrent with Ingestion',
      () async {
        ...
        print(
          '[STRESS TEST STAGE 6 PASSED] 50 rapid refresh calls completed cleanly without state corruption or crashes.',
        );
      },
    );
  });
  ...
  group(
    'STRESS TEST STAGE 7 & 9: Database Lock Contention & Transaction Rollback',
    () {
      test(
        'Transaction Rollback Integrity: Failed Write Leaves Zero Orphans',
        () async {
          ...
          expect(
            txnRows,
            isEmpty,
            reason: 'Transaction table insert must be rolled back on failure',
          );
          expect(
            stateRows,
            isEmpty,
            reason: 'Processing state table must be clean on rollback',
          );
          print(
            '[STRESS TEST STAGE 7 & 9 PASSED] SQLite atomic rollback restored database consistency perfectly.',
          );
        },
      );
    },
  ...
  group('STRESS TEST STAGE 11: Migration Stress (v8 -> v13)', () {
    test(
      'Direct Upgrade from Version 8 Schema to Version 13 with Legacy Rows',
      () async {
        final tempMigrationDir = Directory.systemTemp.createTempSync(
          'migration_stress_',
        );
        ...
        print(
          '[STRESS TEST STAGE 11 PASSED] Version 8 to Version 13 direct upgrade migrated all legacy records cleanly into sms_processing_state.',
        );
      },
    );
  });
  ...
  group('STRESS TEST STAGE 14: Fuzz Testing Parser & Hash Engine', () {
    test(
      '10,000 Fuzzed SMS Inputs: RTL, Unicode, Emojis, Zero-Width Spaces, Corrupted Strings',
      () async {
        ...
        for (int i = 0; i < 10000; i++) {
          ...
          buffer.write(
            ' Rs. ${random.nextInt(50000)}.00 debited from A/C XX${random.nextInt(9999)} ',
          );

          final fuzzedBody = buffer.toString();
          final timestamp = DateTime.now().subtract(
            Duration(minutes: random.nextInt(100000)),
          );
        ...
        expect(successCount, equals(10000));

        print(
          '[STRESS TEST STAGE 14 PASSED] 10,000 Fuzzed inputs processed in ${stopwatch.elapsedMilliseconds}ms (${(stopwatch.elapsedMilliseconds / 10000).toStringAsFixed(2)}ms/op) without any crashes or corrupted state.',
        );
      },
    );
  });
  ...
  group(
    'STRESS TEST STAGE 15: Formal Invariant Verification (10 / 10 Invariants Proven)',
    () {
      test('Invariant 1: One SMS -> Never Two Transactions', () async {
        final timestamp = DateTime(2026, 7, 24, 20, 0);
        const body =
            'Rs. 100.00 debited from A/C XX1111 at TestStore via UPI Ref 77711';
        final hash = SmsTransaction.generateHash(body, timestamp);
      ...
      test('Invariant 2 & 3: Deleted/Ignored SMS -> Never Recreated', () async {
        ...
        const body =
            'Rs. 200.00 debited from A/C XX2222 at Shop via UPI Ref 77722';
        final hash = SmsTransaction.generateHash(body, timestamp);
      ...
      test(
        'Invariant 4: Duplicate Ledger Rows -> Impossible (UNIQUE constraint)',
        () async {
          const hash = 'unique_ledger_hash_test';
          await repository.markSmsProcessingState(
            smsHash: hash,
            status: 'accepted',
          );
          await repository.markSmsProcessingState(
          ...
          expect(
            rows.length,
            equals(1),
            reason: 'UNIQUE(smsHash) must enforce single row per hash',
          );
