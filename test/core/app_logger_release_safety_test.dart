  ...
  group('INVESTMENT-GRADE AUDIT: Logging Pipeline Release Safety', () {
    ...
    test('1. AppLogger formats output correctly in debug environment', () {
      ...
      AppLogger.error('Database connection failed',
          error: 'TimeoutException', label: 'DB');
      ...
      expect(
          capturedLogs[1], contains('[WARN] [Network] High latency detected'));
      expect(
          capturedLogs[2], contains('[ERROR] [DB] Database connection failed'));
      expect(capturedLogs[3], contains('  ↳ TimeoutException'));
    });

    test(
        '2. Global release debugPrint override eliminates raw debugPrint output',
        () {
      ...
      expect(capturedLogs, isEmpty,
          reason: 'Release debugPrint override must drop all messages');
    });

    test(
        '3. AppLogger contains zero raw debugPrint leaks when overridden in release mode',
        () {
      debugPrint = (String? message, {int? wrapWidth}) {};

      AppLogger.info('Sensitive info log', label: 'Test');
      AppLogger.error('Sensitive error log',
          error: Exception('Private data'), label: 'Test');
      AppLogger.debug('Sensitive debug log', label: 'Test');

      expect(capturedLogs, isEmpty,
          reason: 'Release build must yield ZERO logcat outputs');
    });
  });
}
