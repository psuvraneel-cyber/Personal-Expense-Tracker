...
void main() {
  ...
  group('Adversarial QA — Payment Reconciliation & Deduplication', () {
    test(
      'Scenario A & B: User marks bill as paid, subsequent SMS scan preserves single payment history',
      () async {
    ...
    test(
      'Invariant L: Dismissed candidates are persisted and do NOT resurrect on subsequent SMS scans',
      () async {
        final provider = RecurringProvider();
        final now = DateTime(2026, 9, 10);
    ...
    test(
      'Invariant D & Atomicity: markAsPaid atomic SQLite transaction guarantees history and schedule update together',
      () async {
        final repo = RecurringPaymentRepository();
  ...
  group('Adversarial QA — Merchant Normalization & Collision Isolation', () {
    test(
      'Strips UPI-, POS-, BIL/, VPA: prefixes while keeping distinct services isolated',
      () {
        expect(
          RecurringDetectionService.normalizeMerchantName('UPI-NETFLIX'),
          'Netflix',
        );
        expect(
          RecurringDetectionService.normalizeMerchantName('POS-NETFLIX'),
          'Netflix',
        );
        expect(
          RecurringDetectionService.normalizeMerchantName('BIL/NETFLIX'),
          'Netflix',
        );
        expect(
          RecurringDetectionService.normalizeMerchantName('VPA:NETFLIX'),
          'Netflix',
        );
        expect(
          RecurringDetectionService.normalizeMerchantName(
            'Netflix.com India Pvt Ltd',
          ),
          'Netflix',
        );
        ...
        expect(
          RecurringDetectionService.normalizeMerchantName('Amazon'),
          'Amazon',
        );
        expect(
          RecurringDetectionService.normalizeMerchantName('Amazon Prime'),
          'Amazon Prime',
        );
        expect(
          RecurringDetectionService.normalizeMerchantName('Amazon Fresh'),
          'Amazon Fresh',
        );
        expect(
          RecurringDetectionService.normalizeMerchantName('Amazon Pay'),
          'Amazon Pay',
        );
      },
    );
  ...
  group('Adversarial QA — Account Isolation & Sign-out', () {
    test(
      'clearData resets all cached bills and _isInitialized state',
      () async {
        final provider = RecurringProvider();
        await provider.addManual(
  ...
  group('Adversarial QA — Discrete Cashflow Forecast Projections', () {
    test(
      'Weekly bills deduct on all discrete occurrence dates across 30 days without double counting baseline',
      () {
        ...
        // by future recurring bills. Historical variable burn is 30,000 / 31 = 967.74/day.
        // On bill days, drop is variableDaily + 700 = 1667.74.
        expect(dropSep4, closeTo(1667.74, 5.0));
        expect(dropSep11, closeTo(1667.74, 5.0));
        expect(dropSep18, closeTo(1667.74, 5.0));
        expect(dropSep25, closeTo(1667.74, 5.0));
      },
    );
  });
  ...
  group('Adversarial QA — Snooze, Cancel, Reopen & Delete Lifecycle', () {
    test(
      'Snooze advances nextDueAt by 1 cycle, reschedules notification, does NOT log payment history',
      () async {
        final repo = RecurringPaymentRepository();
        final provider = RecurringProvider();
    ...
    test(
      'Cancel keeps history and stops reminders; Delete removes record, history, and reminders permanently',
      () async {
        final repo = RecurringPaymentRepository();
        final provider = RecurringProvider();
        final now = DateTime(2026, 9, 15);
  ...
  group('Adversarial QA — Calendar Boundaries Stress Test', () {
    test(
      '31st day anchor correctly clamped through Feb and restored in 31-day months',
      () {
