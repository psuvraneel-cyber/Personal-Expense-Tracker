  ...
  Future<void> migrateGuestSyncActions(
    ...
    for (int i = 0; i < syncQueue.length; i++) {
      if (syncQueue[i]['userId'] == guestUserId) {
        syncQueue[i] = {...syncQueue[i], 'userId': authenticatedUserId};
      }
    }
  }
...
void main() {
  ...
  group('Transaction Sync Tests (Non-destructive & Offline-first)', () {
    test(
      'A local transaction that does not exist in a bounded Firestore snapshot must remain in SQLite',
      () async {
    ...
    test(
      'A remote snapshot with 1,000 records must not delete the 1,001st local record',
      () async {
    ...
    test(
      'LWW update: newer remote version overwrites local version, older does not',
      () async {
    ...
    test(
      'Outbound Sync Queue: adding a transaction enqueues a create action and triggers processing',
      () async {
        await provider.loadTransactions();
    ...
    test(
      'Outbound Sync Queue: updating and deleting enqueues correct actions',
      () async {
        ...
        expect(repository.syncQueue.isEmpty, isTrue);
        expect(
          firestoreSync.remoteDb
              .firstWhere((x) => x.id == 'txn_to_edit')
              .amount,
          120.0,
        );
        ...
        expect(repository.syncQueue.isEmpty, isTrue);
        expect(
          firestoreSync.remoteDb.any((x) => x.id == 'txn_to_edit'),
          isFalse,
        );
        expect(
          firestoreSync.tombstones.any((x) => x['id'] == 'txn_to_edit'),
          isTrue,
        );
      },
    );
    ...
    test(
      'Offline deletion propagation: remote tombstone deletes local row',
      () async {
        final txn = TransactionRecord(
        ...
        expect(
          repository.db.any((x) => x.id == 'txn_deleted_elsewhere'),
          isFalse,
        );
      },
    );
    ...
    test(
      'Offline deletion propagation: delete wins regardless of clock skew',
      () async {
        final txn = TransactionRecord(
          id: 'txn_skewed',
          amount: 80.0,
        ...
        firestoreSync.tombstoneStreamController.add([
          {
            ...
            'deletedAt': DateTime.now()
                .toIso8601String(), // server timestamp appears older than local clock
          },
        ]);
        ...
        expect(
          repository.syncQueue.any((x) => x['transactionId'] == 'txn_skewed'),
          isFalse,
        );
        expect(
          firestoreSync.tombstones.any((x) => x['id'] == 'txn_skewed'),
          isTrue,
        ); // tombstone preserved
      },
    );
    ...
    test('Three-device deletion propagation', () async {
      ...
      final provB = TransactionProvider(
        repository: repoB,
        firestoreSync: syncB,
      );
      ...
      final provC = TransactionProvider(
        repository: repoC,
        firestoreSync: syncC,
      );
    ...
    test(
      'Compaction: create followed by delete before upload removes action from queue',
      () async {
        await provider.loadTransactions();
    ...
    test(
      'Queue Processor: concurrent triggers process queue exactly once',
      () async {
        await provider.loadTransactions();
        repository.syncQueue.clear();
    ...
    group('Guest-to-Authenticated User Migration', () {
      test(
        'Guest -> authenticated login migrates actions, but User A to User B sign-in isolates data',
        () async {
          repository.syncQueue.clear();
          ...
          final guestAct = repository.syncQueue.firstWhere(
            (x) => x['id'] == 'guest_act_1',
          );
          expect(guestAct['userId'], 'user_b');

          // User A's actions must NOT be migrated to User B
          final userAAct = repository.syncQueue.firstWhere(
            (x) => x['id'] == 'usera_act_1',
          );
    ...
    test(
      'Retry trigger: failed action is kept in queue, incrementing retryCount, and successfully processed on retry trigger',
      () async {
        await provider.loadTransactions();
        ...
        expect(
          repository.syncQueue.first['lastError'],
          contains('Mock network failure'),
        );
    ...
    test(
      'MEDIUM-3: >1000 records in remote Firestore are fetched and synced without silent truncation',
      () async {
